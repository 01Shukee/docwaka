...
interface AuditTrailProps {
  logs?:      AuditLog[];   // optional — defaults to [] so undefined never crashes
  className?: string;
}
    ...
    <Card className={className}>
      ...
      {logs.length === 0 ? (
        ...
        <ol className="relative">
          <div
            className="absolute left-[7px] top-2 bottom-2 w-px bg-tertiary"
            aria-hidden="true"
          />
            ...
            return (
              <li
                ...
                className={[
                  ...
                  index < logs.length - 1 ? "pb-5" : "",
                ].join(" ")}
              >
                <span
                  className="absolute left-0 top-1 w-[15px] h-[15px] rounded-full border-2 border-neutral bg-tertiary flex items-center justify-center shrink-0"
                  aria-hidden="true"
                >
                ...
                <div className="flex-1 min-w-0 -mt-0.5">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className={`text-[13px] font-semibold leading-5 ${config?.color ?? "text-secondary"}`}>
                  ...
                  <p className="text-[12px] text-secondary leading-4 mt-0.5">
